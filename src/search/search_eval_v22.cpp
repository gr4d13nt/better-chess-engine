...
int evaluate_v22_piece(const Board& board) {
  int imp_mg_w = 0;
  int imp_eg_w = 0;
  int imp_mg_b = 0;
  int imp_eg_b = 0;
  ...
  // v21 uses evaluate_layered_split (pawn + king). v22: king from layered, pawn from
  // doubled + extended only — no layered isolated/passed (passed is v21; extended adds
  // backward/islands/candidates).
  int king_mg_w = 0;
  int king_eg_w = 0;
  int king_mg_b = 0;
  int king_eg_b = 0;
  ...
  const int mg_net = (imp_mg_w + king_mg_w + dbl_mg_w + ext_mg_w + knight_mg_w + rook_mg_w +
                      bishop_mg_w + pass_mg_w) -
                     (imp_mg_b + king_mg_b + dbl_mg_b + ext_mg_b + knight_mg_b + rook_mg_b +
                      bishop_mg_b + pass_mg_b);
  const int eg_net = (imp_eg_w + king_eg_w + dbl_eg_w + ext_eg_w + knight_eg_w + rook_eg_w +
                      bishop_eg_w + pass_eg_w) -
                     (imp_eg_b + king_eg_b + dbl_eg_b + ext_eg_b + knight_eg_b + rook_eg_b +
                      bishop_eg_b + pass_eg_b);
...
int evaluate_v22(const Board& board) {
  return evaluate_v22_piece(board) + evaluate_mop_up_net(board);
}
