...
// v27 = v26 (v25 + LMR) with principal-variation search (PVS / null-window).
// Search kernel is isolated in this file so v19–v26 stay on the unchanged paths.
...
bool quiescence_v27(Board& board, int alpha, int beta, SearchState& st, int qdepth, int ply,
                    int& out_score);

bool negamax_v27_tt_impl(Board& board, int depth, int alpha, int beta, SearchState& st, int ply,
...
bool quiescence_v27(Board& board, int alpha, int beta, SearchState& st, int qdepth, int ply,
  ...
  for (const Move& move : moves) {
    ...
    if (!quiescence_v27(board, child_alpha, child_beta, st, qdepth + 1, ply + 1, child_score)) {
    ...
    if (move_index > 0 && score > alpha && score < beta) {
      if (!quiescence_v27(board, -beta, -alpha, st, qdepth + 1, ply + 1, child_score)) {
        board.unmake_move(move, undo);
        return false;
      }
      score = -child_score;
...
bool negamax_v27_tt_impl(Board& board, int depth, int alpha, int beta, SearchState& st, int ply,
  ...
  if (depth == 0) {
    return quiescence_v27(board, alpha, beta, st, 0, ply, out_score);
  }
  ...
  if (allow_null && depth >= kNullMinDepth && extensions == 0 &&
    ...
    if (null_depth >= 1) {
      ...
      int null_score = 0;
      const bool ok =
          negamax_v27_tt_impl(board, null_depth, -beta, -beta + 1, st, ply + 1, 0, false, null_score);
  ...
  for (const Move& move : moves) {
    ...
    const int child_alpha = (move_index == 0) ? -beta : -(alpha + 1);
    const int child_beta = (move_index == 0) ? -alpha : -alpha;
    if (!negamax_v27_tt_impl(board, search_depth, child_alpha, child_beta, st, ply + 1,
    ...
    int score = -child_score;
    if (move_index > 0 && score > alpha && score < beta) {
      if (!negamax_v27_tt_impl(board, child_depth, -beta, -alpha, st, ply + 1, child_extensions,
                               true, child_score)) {
        board.unmake_move(move, undo);
        return false;
      }
...
bool negamax_v27_tt(Board& board, int depth, int alpha, int beta, SearchState& st, int& out_score) {
  return negamax_v27_tt_impl(board, depth, alpha, beta, st, 0, 0, true, out_score);
}
