...
// Optimistic capture gain must clear alpha by this much (centipawns).
...
bool prune_delta_qcapture(int stand_pat, int alpha, const Board& board, const Move& m) {
  ...
  const int capture_value = captured_piece_value(board, m);
  return stand_pat + capture_value + kDeltaMargin < alpha;
}
...
bool quiescence_ordered(Board& board, int alpha, int beta, SearchState& st, int qdepth,
                        int& out_score) {
  if (should_stop(st)) {
    return false;
  }
  ...
  for (const Move& move : moves) {
    ...
    int child_score = 0;
    if (!quiescence_ordered(board, -beta, -alpha, st, qdepth + 1, child_score)) {
...
bool negamax_v17_tt_impl(Board& board, int depth, int alpha, int beta, SearchState& st,
  ...
  if (depth == 0) {
    return quiescence_ordered(board, alpha, beta, st, 0, out_score);
  }
  ...
  for (const Move& move : moves) {
    ...
    int child_score = 0;
    if (!negamax_v17_tt_impl(board, child_depth, -beta, -alpha, st, child_extensions,
                             child_score)) {
      board.unmake_move(move, undo);
      return false;
    }
...
bool negamax_v17_tt(Board& board, int depth, int alpha, int beta, SearchState& st, int& out_score) {
  return negamax_v17_tt_impl(board, depth, alpha, beta, st, 0, out_score);
}
