...
// v30 = v29 search kernel (v28 LMP + v27 PVS) + futility pruning at depth 1–3.
...
constexpr int kLmpMinDepth = 2;
constexpr int kLmpMaxDepth = 3;

constexpr int kFutilityMinDepth = 1;
constexpr int kFutilityMaxDepth = 3;

int lmp_move_threshold(int depth) {
...
bool should_lmp(const Board& board, const Move& move, int depth, int move_index, int alpha,
                int beta, bool in_check, int extensions, int legal_count) {
  (void)alpha;
  if (beta - alpha <= 1 || in_check || extensions > 0 || depth < kLmpMinDepth ||
      depth > kLmpMaxDepth) {
    return false;
  }
  if (is_tactical_move(move) || gives_check(board, move)) {
    return false;
  }
  const int threshold = lmp_move_threshold(depth);
  return legal_count > threshold && move_index >= threshold;
}

bool should_futility_prune(int static_eval, const Board& board, const Move& move, int depth,
                           int move_index, int alpha, int beta, bool in_check, int extensions) {
  (void)beta;
  if (beta - alpha > 1 || in_check || extensions > 0 || move_index == 0 ||
      depth < kFutilityMinDepth || depth > kFutilityMaxDepth) {
    return false;
  }
  if (is_tactical_move(move) || gives_check(board, move)) {
    return false;
...
bool should_reverse_futility(int static_eval, int depth, int alpha, int beta, bool in_check,
                             int extensions) {
  if (beta - alpha <= 1 || in_check || extensions > 0 || depth < kFutilityMinDepth ||
      depth > kFutilityMaxDepth) {
    return false;
  }
  return static_eval - futility_margin(depth) >= beta;
}
...
bool quiescence_v30(Board& board, int alpha, int beta, SearchState& st, int qdepth, int ply,
                    int& out_score);

bool negamax_v30_tt_impl(Board& board, int depth, int alpha, int beta, SearchState& st, int ply,
...
bool quiescence_v30(Board& board, int alpha, int beta, SearchState& st, int qdepth, int ply,
  ...
  for (const Move& move : moves) {
    ...
    int child_score = 0;
    const int child_alpha = (move_index == 0) ? -beta : -(alpha + 1);
    const int child_beta = (move_index == 0) ? -alpha : -alpha;
    if (!quiescence_v30(board, child_alpha, child_beta, st, qdepth + 1, ply + 1, child_score)) {
    ...
    if (move_index > 0 && score > alpha && score < beta) {
      if (!quiescence_v30(board, -beta, -alpha, st, qdepth + 1, ply + 1, child_score)) {
        board.unmake_move(move, undo);
        return false;
      }
      score = -child_score;
...
bool negamax_v30_tt_impl(Board& board, int depth, int alpha, int beta, SearchState& st, int ply,
  ...
  int tt_score = 0;
  if (depth > 0 && beta - alpha > 1 && st.tt != nullptr &&
  ...
  if (depth == 0) {
    return quiescence_v30(board, alpha, beta, st, 0, ply, out_score);
  }
  ...
  if (allow_null && depth >= kNullMinDepth && extensions == 0 &&
    ...
    if (null_depth >= 1) {
      ...
      int null_score = 0;
      const bool ok =
          negamax_v30_tt_impl(board, null_depth, -beta, -beta + 1, st, ply + 1, 0, false, null_score);
  ...
  const int static_eval =
      in_check ? -kMateScore : evaluate_for_side_to_move(board, st.eval_profile);
  if (should_reverse_futility(static_eval, depth, alpha, beta, in_check, extensions)) {
    out_score = beta;
    if (st.tt != nullptr && beta - alpha > 1) {
      st.tt->store(key, depth, beta, TTBound::Lower, st.tt_generation);
    }
    return true;
  }
  ...
  int move_index = 0;
  bool quiet_pruned = false;
  for (const Move& move : moves) {
    if (should_futility_prune(static_eval, board, move, depth, move_index, alpha, beta, in_check,
                              extensions)) {
      quiet_pruned = true;
      ++move_index;
      continue;
    }
    if (should_lmp(board, move, depth, move_index, alpha, beta, in_check, extensions,
    ...
    const int child_alpha = (move_index == 0) ? -beta : -(alpha + 1);
    const int child_beta = (move_index == 0) ? -alpha : -alpha;
    if (!negamax_v30_tt_impl(board, search_depth, child_alpha, child_beta, st, ply + 1,
                             child_extensions, true, child_score)) {
      board.unmake_move(move, undo);
      return false;
    }
    ...
    int score = -child_score;
    if (move_index > 0 && score > alpha && score < beta) {
      if (!negamax_v30_tt_impl(board, child_depth, -beta, -alpha, st, ply + 1, child_extensions,
                               true, child_score)) {
        board.unmake_move(move, undo);
        return false;
      }
...
bool negamax_v30_tt(Board& board, int depth, int alpha, int beta, SearchState& st, int& out_score) {
  return negamax_v30_tt_impl(board, depth, alpha, beta, st, 0, 0, true, out_score);
}
