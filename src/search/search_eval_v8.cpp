...
int evaluate_v8(const Board& board) {
  ...
  int out_mg_b = 0;
  int out_eg_b = 0;
  evaluate_knight_outposts_split(board, out_mg_w, out_eg_w, out_mg_b, out_eg_b);

  const int phase = game_phase(board);
  const int mg_net = (imp_mg_w + lay_mg_w + out_mg_w) - (imp_mg_b + lay_mg_b + out_mg_b);
  const int eg_net = (imp_eg_w + lay_eg_w + out_eg_w) - (imp_eg_b + lay_eg_b + out_eg_b);
