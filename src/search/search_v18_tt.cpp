...
bool negamax_v18_tt_impl(Board& board, int depth, int alpha, int beta, SearchState& st,
  ...
  for (const Move& move : moves) {
    ...
    int child_score = 0;
    if (!negamax_v18_tt_impl(board, child_depth, -beta, -alpha, st, child_extensions, true,
                             child_score)) {
      board.unmake_move(move, undo);
      return false;
    }
...
bool negamax_v18_tt(Board& board, int depth, int alpha, int beta, SearchState& st, int& out_score) {
  return negamax_v18_tt_impl(board, depth, alpha, beta, st, 0, true, out_score);
}
