...
int evaluate_v9(const Board& board) {
  ...
  const int mg_net = (imp_mg_w + lay_mg_w + knight_mg_w + rook_mg_w) -
                     (imp_mg_b + lay_mg_b + knight_mg_b + rook_mg_b);
  const int eg_net = (imp_eg_w + lay_eg_w + knight_eg_w + rook_eg_w) -
                     (imp_eg_b + lay_eg_b + knight_eg_b + rook_eg_b);
