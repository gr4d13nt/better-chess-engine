...
bool quiescence_ordered(Board& board, int alpha, int beta, SearchState& st, int qdepth, int ply,
                        int& out_score) {
  if (should_stop(st)) {
    return false;
  }
  ...
  for (const Move& move : moves) {
    ...
    int child_score = 0;
    if (!quiescence_ordered(board, -beta, -alpha, st, qdepth + 1, ply + 1, child_score)) {
...
bool negamax_v19_tt_impl(Board& board, int depth, int alpha, int beta, SearchState& st, int ply,
  ...
  if (depth == 0) {
    return quiescence_ordered(board, alpha, beta, st, 0, ply, out_score);
  }
  ...
  if (allow_null && depth >= kNullMinDepth && extensions == 0 &&
    ...
    if (null_depth >= 1) {
      ...
      int null_score = 0;
      const bool ok =
          negamax_v19_tt_impl(board, null_depth, -beta, -beta + 1, st, ply + 1, 0, false, null_score);
  ...
  for (const Move& move : moves) {
    ...
    int child_score = 0;
    if (!negamax_v19_tt_impl(board, child_depth, -beta, -alpha, st, ply + 1, child_extensions, true,
                             child_score)) {
      board.unmake_move(move, undo);
      return false;
    }
...
bool negamax_v19_tt(Board& board, int depth, int alpha, int beta, SearchState& st, int& out_score) {
  return negamax_v19_tt_impl(board, depth, alpha, beta, st, 0, 0, true, out_score);
}
